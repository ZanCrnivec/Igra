...
class StaminaBar():
    ...
    def draw(self, display):    
       ...
       # pygame.draw.rect(display, "red", (self.x + 4, self.y + 4, (self.width) - 8, self.height - 8))
        pygame.draw.rect(display, "white", (self.x + 4, self.y + 4, (self.width * ratio) - 8, self.height - 8))

class BatteryBar():
    def __init__(self, x, y, width, height, max_battery):
        self.x = x
        self.y = y
        self.width = width
        self.height = height
    ...
    def draw(self, display):    
        ratio = self.battery / self.max_battery
  #      pygame.draw.rect(display, "red", (self.x, self.y, self.width, self.height))
        pygame.draw.rect(display, "green", (self.x, self.y, self.width * ratio, self.height))
...
class Tema:
    def __init__(self, width, height, image1, image2):
        self.width = width
        self.height = height
        self.original_image1 = pygame.transform.scale(image1, (self.width, self.height))
        self.original_image2 = pygame.transform.scale(image2, (self.width, self.height))
        self.image = self.original_image1

    def draw(self, display, alpha, player_x, player_y, angle):
        temp_surface = pygame.transform.rotate(self.image, angle)
        temp_surface.set_alpha(alpha)
        rotated_rect = temp_surface.get_rect(center=(player_x, player_y))
        display.blit(temp_surface, rotated_rect.topleft)

    def update_image(self, mouse_button_down):
        if mouse_button_down and battery_bar.battery > 0:
            self.image = self.original_image1
        else:
...
battery_bar = BatteryBar(325, 650, 400, 20, 100)
...
display_scroll = [0, 0]
timer = 30
tema_image1 = pygame.image.load("tema5.png")
tema_image2 = pygame.image.load("tema4.png")


tema = Tema(1500, 1500, tema_image1, tema_image2)
score = 0
svetloba = 255
...
while True:
    ...
    tema.update_image(mouse_button_down)
